...
想查询到对应的语句，可以在 `SHOW profile FOR query 对应的 Query_ID;` 来查看
...
我们可以通过 `SHOW DATABASES;` 来查看数据库，我们发现有 `performance_schema` 这个数据库
...
- summary 表提供所有事件的汇总信息，该组中的表以不同的方式汇总事件数据（如：按用户，按主机，按线程等等）
